...
for ($i = 0; $i -lt 10; $i++) {
    ...
    foreach ($w in $root.FindAll([System.Windows.Automation.TreeScope]::Descendants, [System.Windows.Automation.Condition]::TrueCondition)) {
...
$dlgEls = $dlg.FindAll([System.Windows.Automation.TreeScope]::Descendants, [System.Windows.Automation.Condition]::TrueCondition)
Write-Output "Elements: $($dlgEls.Count)"
...
foreach ($el in $dlgEls) {
    ...
    if ($type -eq "ControlType.CheckBox") {
        ...
        Write-Output "CHK N=[$n] ID=[$aid] log=($lx,$ly,$lr,$lb) ctr=($cx,$cy) st=$chkVal"
    }
    if ($type -eq "ControlType.ComboBox" -or $type -eq "ControlType.Edit") {
        ...
        if ($v -ne "") { Write-Output "EDIT N=[$n] V=[$v] log=($lx,$ly,$lr,$lb) ctr=($cx,$cy)" }
    }
    if ($type -eq "ControlType.Button") {
        if ($n -match "OK|Cancel|Refresh|Populate") {
            Write-Output "BTN N=[$n] log=($lx,$ly,$lr,$lb) ctr=($cx,$cy)"
        }
    }
}
